...
- `docker/` — Dockerfile, compose.yaml, entrypoint, CUPS/SANE/Avahi configs
  - `cups/papyrus-backend` — Custom CUPS backend script for network print → hold queue. Posts the job to the ingest API with `curl`: text fields (title/username/etc., all untrusted from CUPS) go through **`--form-string`, never `-F`** — `-F` treats a value starting with `@`/`<` as read-from-file and `;` as a param separator, so a document title like `@notes` made curl abort with HTTP 000 and the job vanished; the file part uses a **fixed `filename=document.pdf`** (real title is preserved in the `title` field). Retries on HTTP 000/5xx (~10× / 3s, rides through an API restart), does not retry 4xx, and on final failure `exit 1`s (`CUPS_BACKEND_FAILED`) so failures surface as failed jobs instead of silently completing — safe only because every Papyrus-created queue uses `printer-error-policy=abort-job` (aborts the one job, never disables the queue). Keeps the temp `.pdf` under `PAPYRUS_UPLOAD_DIR` for debugging. Regression-tested by `tests/test_cups_backend_script.py` (shims `curl`).
